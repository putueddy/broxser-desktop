        ...
        match event {
            ...
            zwp_text_input_v3::Event::CommitString { text } => {
                ...
                if let Some(commit_text) = text {
                    drop(state);
                    // Broxser: a commit is text, including a single ASCII byte.
                    // Synthesizing a key-down here gives the application no way
                    // to distinguish it from a physical key with a later key-up.
